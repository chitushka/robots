--[[
rsi_mean_reversion_robot.lua

Торговый робот для терминала QUIK на чистом QLua.
Стратегия: RSI Mean Reversion — возврат к среднему через RSI.

Логика стратегии:
1. BUY:
   RSI(period) < buy_level.
   По умолчанию: RSI14 < 25.

2. EXIT:
   RSI(period) > exit_level.
   По умолчанию: RSI14 > 50.
...
- rsi_mean_reversion_robot.log
- rsi_mean_reversion_signals.csv
...
Config = {
    robot_name = "RSI_MEAN_REVERSION_ROBOT",
    ...
    -- Параметры RSI Mean Reversion.
    rsi_period = 14,
    buy_level = 25,
    exit_level = 50,

    -- Дополнительный фильтр: не покупать, если инструмент падает слишком резко.
    -- Для первой версии выключен.
    use_price_drop_filter = false,
    max_drop_percent_from_prev_close = 5.0,
    ...
    -- Логи.
    log_file = "rsi_mean_reversion_robot.log",
    signals_file = "rsi_mean_reversion_signals.csv",
    trades_file = "rsi_mean_reversion_trades.csv",
...
function Logger.signal(signal)
    local line = table.concat({
        ...
        tostring(signal.price_drop_percent or ""),
        signal.reason or ""
    }, ";")
...
function Logger.init_files()
    if not file_exists(Config.signals_file) then
        Logger.append(
            Config.signals_file,
            "datetime;strategy;class_code;sec_code;action;price;lots;rsi;prev_rsi;price_drop_percent;reason"
        )
    end
...
function Indicators.price_drop_percent_from_previous_close(candles)
    if candles == nil or #candles < 2 then
        return nil
    end
    ...
    if prev_close == nil or prev_close == 0 or close_price == nil then
        return nil
    end

    return ((prev_close - close_price) / prev_close) * 100
end

----------------------------------------------------------------------
-- 7. STRATEGY: RSI MEAN REVERSION
----------------------------------------------------------------------
...
function Strategy.evaluate(candles)
    ...
    if rsi[i] == nil or rsi[prev] == nil then
        return {
            strategy = "rsi_mean_reversion",
            action = "HOLD",
    ...
    local price_drop_percent = Indicators.price_drop_percent_from_previous_close(candles)

    local action = "HOLD"
    local reason = "no RSI mean reversion signal"

    -- Дополнительная защита от резкого падения.
    -- Если включена, блокирует входы при слишком сильном падении за последнюю свечу.
    local drop_filter_blocked = false
    if Config.use_price_drop_filter and price_drop_percent ~= nil then
        if price_drop_percent > Config.max_drop_percent_from_prev_close then
            drop_filter_blocked = true
        end
    end

    -- BUY:
    -- Вход на перепроданности.
    -- Для уменьшения количества повторных сигналов используем условие:
    -- предыдущий RSI был >= buy_level, текущий стал < buy_level.
    -- То есть реагируем именно на вход RSI в зону перепроданности.
    if prev_rsi >= Config.buy_level and current_rsi < Config.buy_level then
        if drop_filter_blocked then
            action = "HOLD"
            reason = "RSI buy blocked by price drop filter: drop=" .. tostring(price_drop_percent)
        else
            action = "BUY"
            reason = "RSI" .. tostring(Config.rsi_period) ..
                     " crossed below buy_level " .. tostring(Config.buy_level) ..
                     ": prev=" .. tostring(prev_rsi) ..
                     " current=" .. tostring(current_rsi)
        end
    end

    -- EXIT:
    -- Выход при возврате RSI выше exit_level.
    -- Используем пересечение снизу вверх, чтобы не повторять EXIT на каждой свече.
    if prev_rsi <= Config.exit_level and current_rsi > Config.exit_level then
        action = "EXIT"
        reason = "RSI" .. tostring(Config.rsi_period) ..
                 " crossed above exit_level " .. tostring(Config.exit_level) ..
                 ": prev=" .. tostring(prev_rsi) ..
                 " current=" .. tostring(current_rsi)
    end
...
function handle_signal(signal)
    ...
    if signal.action == "HOLD" then
        Logger.info(
            ...
            " prev_rsi=" .. tostring(signal.prev_rsi) ..
            " price_drop_percent=" .. tostring(signal.price_drop_percent)
        )
        return
    end
...
function validate_config()
    ...
    local required_min = Config.rsi_period + 5

    if Config.min_candles < required_min then
        return false, "min_candles must be at least rsi_period + 5"
    end
...
function init()
    ...
    Logger.info(
        ...
        " use_price_drop_filter=" .. bool_to_string(Config.use_price_drop_filter)
    )
