--[[
ema_crossover_robot.lua

Торговый робот для терминала QUIK на чистом QLua.
Стратегия: EMA Crossover — пересечение быстрой и медленной EMA.
...
- ema_crossover_robot.log
- ema_crossover_signals.csv
- ema_crossover_trades.csv
...
Config = {
    robot_name = "EMA_CROSSOVER_ROBOT",
    ...
    -- Нужно больше slow_ema_period, чтобы EMA успела стабилизироваться.
    min_candles = 120,
    candles_to_read = 250,

    -- Параметры заявки.
    lots = 1,
    order_type = "L", -- L = лимитная заявка. M = рыночная, но для первой версии не использовать.
    ...
    -- Логи.
    log_file = "ema_crossover_robot.log",
    signals_file = "ema_crossover_signals.csv",
    trades_file = "ema_crossover_trades.csv",
...
function Logger.init_files()
    if not file_exists(Config.signals_file) then
        Logger.append(Config.signals_file, "datetime;strategy;class_code;sec_code;action;price;lots;reason")
    end

    if not file_exists(Config.trades_file) then
        Logger.append(Config.trades_file, "datetime;mode;strategy;class_code;sec_code;action;price;lots;pnl;reason")
    end
end
...
function Strategy.evaluate(candles)
    ...
    if #closes < Config.min_candles then
        return {
    ...
    local fast = Indicators.ema_series(closes, Config.fast_ema_period)
    local slow = Indicators.ema_series(closes, Config.slow_ema_period)

    local i = #closes
    local prev = i - 1

    if fast[prev] == nil or slow[prev] == nil or fast[i] == nil or slow[i] == nil then
        return {
            strategy = "ema_crossover",
            action = "HOLD",
    ...
    local price = closes[i]
    local action = "HOLD"
    local reason = "no crossover"

    -- BUY: быстрая EMA пересекла медленную снизу вверх.
    if fast[prev] <= slow[prev] and fast[i] > slow[i] then
        action = "BUY"
        reason = "EMA" .. tostring(Config.fast_ema_period) ..
                 " crossed EMA" .. tostring(Config.slow_ema_period) .. " up"
    end

    -- EXIT: быстрая EMA пересекла медленную сверху вниз.
    if fast[prev] >= slow[prev] and fast[i] < slow[i] then
        action = "EXIT"
        reason = "EMA" .. tostring(Config.fast_ema_period) ..
                 " crossed EMA" .. tostring(Config.slow_ema_period) .. " down"
    end
    ...
    return {
        ...
        fast_ema = fast[i],
        slow_ema = slow[i],
        prev_fast_ema = fast[prev],
        prev_slow_ema = slow[prev],
...
function Orders.send_live_order(signal)
    ...
    -- Для production нужно сверять сделки через таблицы QUIK.
    if signal.action == "BUY" then
...
function handle_signal(signal)
    ...
    if signal.action == "HOLD" then
        Logger.info(
            ...
            " fast=" .. tostring(signal.fast_ema) ..
            " slow=" .. tostring(signal.slow_ema)
        )
        return
    end
...
function validate_config()
    if Config.fast_ema_period <= 0 then
        return false, "fast_ema_period must be positive"
    end

    if Config.slow_ema_period <= 0 then
        return false, "slow_ema_period must be positive"
    end

    if Config.fast_ema_period >= Config.slow_ema_period then
...
function init()
    ...
    Logger.info("EMA fast=" .. tostring(Config.fast_ema_period) .. " slow=" .. tostring(Config.slow_ema_period))
