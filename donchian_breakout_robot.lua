--[[
donchian_breakout_robot.lua

Торговый робот для терминала QUIK на чистом QLua.
Стратегия: Donchian Channel Breakout — пробой максимума/минимума за период.
...
- donchian_breakout_robot.log
- donchian_breakout_signals.csv
- donchian_breakout_trades.csv
...
Config = {
    robot_name = "DONCHIAN_BREAKOUT_ROBOT",
    ...
    -- Логи.
    log_file = "donchian_breakout_robot.log",
    signals_file = "donchian_breakout_signals.csv",
    trades_file = "donchian_breakout_trades.csv",
...
function Logger.init_files()
    if not file_exists(Config.signals_file) then
        Logger.append(Config.signals_file, "datetime;strategy;class_code;sec_code;action;price;lots;entry_high;exit_low;reason")
    end
...
----------------------------------------------------------------------
-- 7. STRATEGY: DONCHIAN CHANNEL BREAKOUT
----------------------------------------------------------------------
...
function Strategy.evaluate(candles)
    ...
    -- Важно:
    -- Канал считается по ПРЕДЫДУЩИМ свечам, без текущей свечи.
    -- Иначе текущий High/Low будет попадать в канал и ломать факт пробоя.
    ...
    if entry_high == nil or exit_low == nil then
        return {
            strategy = "donchian_breakout",
            action = "HOLD",
            reason = "Donchian channel is not ready"
        }
    end
    ...
    if close_price > entry_high then
        action = "BUY"
        reason = "Close " .. tostring(close_price) ..
                 " > Donchian High" .. tostring(Config.entry_period) ..
                 " " .. tostring(entry_high)
    elseif close_price < exit_low then
        action = "EXIT"
        reason = "Close " .. tostring(close_price) ..
                 " < Donchian Low" .. tostring(Config.exit_period) ..
                 " " .. tostring(exit_low)
    end
...
function validate_config()
    ...
    if Config.min_candles < Config.entry_period + 2 then
        return false, "min_candles must be at least entry_period + 2"
    end
...
function init()
    ...
    Logger.info("Donchian entry_period=" .. tostring(Config.entry_period) .. " exit_period=" .. tostring(Config.exit_period))
